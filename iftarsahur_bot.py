...
async def a(client, message):
    ...
    try:
        ...
        for i in range(len(dur_arr)-1, -1, -1):
            dur += (int(dur_arr[i]) * secmul)
            secmul *= 60
        await message.reply_video(chat_id=update.chat.id, video=dldir, caption=file_name, duration=duration, reply_to_message_id=update.message_id, progress=progress_for_pyrogram)
        await m.delete()
...
@app.on_message(f.command('ziyagil'))
...
@app.on_message(f.command('azgin'))
